...
const runChild = async (
  ...
  child.on('close', (code) => {
    ...
    clearTimeout(timer)
    if (forceKillTimer) clearTimeout(forceKillTimer)
    const shuttingDown = tracker.isShuttingDown()
    if (timedOut || shuttingDown) {
      // A descendant can be forked after the initial group signal was
      // delivered but before the group leader exits. Once `close` fires the
      // leader can no longer create descendants, so this final group sweep
      // closes that race before the run settles and its workspace is removed.
      killProcessTree(child.pid, 'SIGKILL', () => child.kill('SIGKILL'))
    }
    if (shuttingDown) {
...
const serviceName = (scope: AgentCredentialScope): string => {
  ...
  // This digest is only a stable, non-secret Keychain namespace identifier; it
  // never processes a password and is not used for authentication or integrity.
