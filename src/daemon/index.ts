...
const routeRequest = async (context: RouteContext): Promise<void> => {
  ...
  if (method === 'POST' && path === '/v1/connections/openauth/disconnect') {
    assertNativeApproval(request, DAEMON_NATIVE_APPROVAL.disconnectOpenAuth)
    writeSuccess(response, await context.decorateSnapshot(await service.disconnectOpenAuth()))
    return
  }
  ...
  if (method === 'POST' && path === '/v1/dima/preview') {
    writeSuccess(response, await service.previewDimaRequirement(assertDimaRequirementInput(await readJsonBody(request))))
    return
  }
  if (method === 'POST' && path === '/v1/dima/create') {
    writeSuccess(response, await service.createDimaRequirement(assertDimaRequirementCreateInput(await readJsonBody(request))))
    return
  }
