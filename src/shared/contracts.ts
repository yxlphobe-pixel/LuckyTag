export const IPC_CHANNELS = {
  ...
  disconnectOpenAuth: 'luckytag:disconnect-openauth',
  startWorker: 'luckytag:start-worker',
  stopWorker: 'luckytag:stop-worker',
  runOnce: 'luckytag:run-once',
  previewDimaRequirement: 'luckytag:preview-dima-requirement',
  createDimaRequirement: 'luckytag:create-dima-requirement',
  openDimaRequirement: 'luckytag:open-dima-requirement',
  revealRuntimeFolder: 'luckytag:reveal-runtime-folder',
  snapshotUpdated: 'luckytag:snapshot-updated'
} as const

export type ConnectionKind = 'dingtalk' | 'yuque' | 'openauth' | 'ding-a1'
...
export type DimaChatWindow = '24h' | '3d' | '7d'

export interface DimaRequirementInput {
  groupName: string
  chatWindow: DimaChatWindow
  dimaSpace: string
  iteration: string
  templateUrl: string
}

export interface DimaRequirementEvidence {
  sentAt: string
  sender: string
  text: string
}

export interface DimaRequirementPreview {
  draftId: string
  title: string
  description: string
  groupName: string
  ...
  windowEnd: string
  messageCount: number
  evidence: DimaRequirementEvidence[]
  spaceId: string
  spaceName: string
...
export interface DimaRequirementCreateInput {
  draftId: string
  title: string
  description: string
}

export interface DimaRequirementResult {
  workItemId: string
  title: string
  url: string
  spaceId: string
...
export interface LuckyTagApi {
  ...
  authenticate(kind: 'dingtalk' | 'yuque' | 'openauth'): Promise<ApiResult<ConnectionStatus>>
  disconnectOpenAuth(): Promise<ApiResult<DashboardSnapshot>>
  ...
  previewDimaRequirement(input: DimaRequirementInput): Promise<ApiResult<DimaRequirementPreview>>
  createDimaRequirement(input: DimaRequirementCreateInput): Promise<ApiResult<DimaRequirementResult>>
  openDimaRequirement(url: string): Promise<ApiResult<null>>
