...
import type { SampleLibraryAuthStatus, SampleLibraryClient, SampleLibraryDocument } from './types'
import { findArray, findBoolean, findString, isRecord, unwrapEnvelope } from './value-utils'
export class SampleLibraryAdapter implements SampleLibraryClient {
  private selectedExecutable: string
  constructor(
    private readonly runner: JsonCliRunner,
    executable = 'sample-library-mock',
    private readonly fallbackExecutable = executable === 'sample-library-mock' ? undefined : 'sample-library-fixture',
    private readonly wait: (delayMs: number) => Promise<void> = waitFor,
    private readonly now: () => number = Date.now,
    private readonly environment: Readonly<Record<string, string>> = buildSampleLibraryCliEnvironment()
  ) {
    this.selectedExecutable = executable
  }
  async probe(): Promise<SampleLibraryAuthStatus> {
    const response = await this.run(['status'], undefined, { SAMPLE_MOCK_MODE: '1' })
    return normalizeAuthStatus(response, '示例知识库 CLI 已连接')
  }
  async authenticate(): Promise<SampleLibraryAuthStatus> {
    // Plain status reuses a valid session and starts browser authorization when no
    ...
    return normalizeAuthStatus(response, '示例知识库认证完成')
  }
  async resolve(input: string): Promise<string> {
    if (!input.trim()) throw new Error('示例知识库地址不能为空')
    const response = await this.run(['resolve', input])
    const payload = unwrapEnvelope(response, 'SampleLibrary resolve')
    const route = extractRoute(payload)
    if (!route) throw new Error('示例知识库 resolve 未返回可用文档路径')
    return route
  }
  async listDocuments(namespace: string): Promise<Array<{ route: string; title?: string }>> {
    if (!namespace.trim()) throw new Error('示例知识库知识库 namespace 不能为空')
    ...
    while (offset < MAX_LIST_DOCUMENTS) {
      ...
      const payload = unwrapEnvelope(response, 'SampleLibrary list docs')
  ...
  async showDocument(route: string): Promise<SampleLibraryDocument> {
    if (!route.trim()) throw new Error('示例知识库文档路径不能为空')
    const response = await this.run(['show', 'doc', route])
    const payload = unwrapEnvelope(response, 'SampleLibrary show doc')
    const body = extractBody(payload)
    if (!body.trim()) throw new Error(`示例知识库文档正文为空：${route}`)
  ...
  private async run(
    ...
    if (this.selectedExecutable === 'sample-library-mock') {
      if (args[0] === 'resolve') return { success: true, data: { route: 'public-demo/library/sample-document' } }
      if (args[0] === 'list') return { success: true, data: [] }
      if (args[0] === 'show') return { success: true, data: { route: String(args[2] ?? 'public-demo/library/sample-document'), title: 'Public mock document', body: 'This local-only document contains no external integration logic.' } }
      return { success: true, data: { authenticated: false, detail: 'Public mock connector only', version: '2.0.0-mock' } }
    }
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          throw normalizeSampleLibraryCliError(fallbackError)
        }
      }
      throw normalizeSampleLibraryCliError(error)
    }
  }
...
const normalizeSampleLibraryCliError = (error: unknown): unknown => {
  if (!(error instanceof Error)) return error
  const message = error.message
  if (error instanceof CliExecutionError && error.exitCode === 'ETIMEDOUT') {
    return Object.assign(
      new Error(
        '示例知识库请求超时。请确认已连接本地测试网络；若正在授权，请在浏览器打开后 2 分钟内完成，再返回 LuckyTag 重试',
        { cause: error }
      ),
  ...
  if (/Authorization(?:\s+request\s+failed\s*:?)?\s*timeout\b/iu.test(message)) {
    return Object.assign(
      new Error('示例知识库授权超时。请在浏览器打开后 2 分钟内完成授权，再返回 LuckyTag 重试', {
        cause: error
      }),
      { code: 'SAMPLE_LIBRARY_AUTH_TIMEOUT' }
    )
  ...
  if (
    ...
    return Object.assign(
      new Error(
        `示例知识库授权服务连接失败${networkCode ? `（${networkCode}）` : ''}。请确认已连接本地测试网络后重试`,
        { cause: error }
      ),
...
export const buildSampleLibraryCliEnvironment = (
  ...
  // SampleTrust CA only for SampleLibrary's Node process; do not change TLS verification or
  // the trust environment of DEMO_MESSAGE/SampleAuth.
...
const normalizeAuthStatus = (response: unknown, defaultDetail: string): SampleLibraryAuthStatus => {
  const payload = unwrapEnvelope(response, 'SampleLibrary status')
