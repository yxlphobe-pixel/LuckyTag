...
import type { CoreStateFile, DemoMessageClient, DemoMessageMessage, PersistedReplyRecord } from './types'
import { errorMessage } from './value-utils'

export interface WorkerEngineOptions {
  stateStore: JsonStore<CoreStateFile>
  demoMessage: DemoMessageClient
...
export class WorkerEngine {
  ...
  private readonly demoMessage: DemoMessageClient
  ...
  constructor(options: WorkerEngineOptions) {
    ...
    this.demoMessage = options.demoMessage
  ...
  private async executeRun(): Promise<RunSummary> {
    ...
    try {
      ...
      this.demoMessage.setAllowlistedGroups(groups.map((group) => group.id))
  ...
  private async fetchMentions(groupId: string, start: string, end: string): Promise<DemoMessageMessage[]> {
    ...
    for (let pageNumber = 0; pageNumber < 10; pageNumber += 1) {
      ...
      const page = await this.demoMessage.listMentions({ groupId, start, end, limit: 30, cursor })
  ...
  private async processOpenFollowUps(
    ...
    for (const window of [...newestBySender.values()].slice(0, 10)) {
      const history = await this.demoMessage.listConversationMessages({
        groupId: group.id,
    ...
    for (const record of records) {
      if (now - new Date(record.updatedAt).getTime() >= 24 * 60 * 60_000) {
        await this.setStatus(record.id, 'needs_manual', {
          reason: '未完成发送已超过 DEMO_MESSAGE 24 小时幂等窗口，为避免重复消息已转交人工'
        })
  ...
  private async processRecord(
    ...
    const stopWithoutRetry = async (
      ...
      if (hadUncertainSend) {
        await this.setStatus(record.id, 'needs_manual', {
          ...patch,
          reason: `此前 DEMO_MESSAGE 发送结果不确定，不能确认消息是否已送达；${reason}，请人工核对`
        })
        return 'needs_manual'
      }
    ...
    let history: DemoMessageMessage[]
    try {
      const historyStart = new Date(new Date(mention.createdAt).getTime() - 2 * 60_000)
      history = await this.demoMessage.listConversationMessages({
        groupId: group.id,
    ...
    if (config.replyPolicy.dryRun) {
      return stopWithoutRetry('dry_run', 'Dry-run：已生成预览，没有发送到示例消息', {
        reply,
        evidence
      })
    }
    ...
    if (latestConfig.replyPolicy.dryRun) {
      return stopWithoutRetry('dry_run', '发送前已切换为预演模式；已生成预览，没有发送到示例消息', {
        reply,
        evidence
      })
    }
    ...
    let finalHistory: DemoMessageMessage[]
    try {
      const finalHistoryStart = new Date(new Date(mention.createdAt).getTime() - 2 * 60_000)
      finalHistory = await this.demoMessage.listConversationMessages({
        groupId: group.id,
    ...
    if (finalConfig.replyPolicy.dryRun) {
      ...
      return stopWithoutRetry('dry_run', '发送前最终检查已切换为预演模式；没有发送到示例消息', {
        reply,
        evidence
      })
    }
    ...
    try {
      ...
      if (immediateConfig) {
        ...
        if (!immediateConfig.replyPolicy.enabled || !immediateGroupAllowed) {
          return stopWithoutRetry(
            'ignored',
            '调用 DEMO_MESSAGE 前检测到自动回复已关闭或群已移出白名单，已取消发送'
          )
        }
        if (immediateConfig.replyPolicy.dryRun) {
          return stopWithoutRetry(
            'dry_run',
            '调用 DEMO_MESSAGE 前检测到已切换为预演模式；没有发送到示例消息'
          )
        }
        ...
        if (
          ...
          await this.setStatus(record.id, 'needs_manual', {
            reason: '调用 DEMO_MESSAGE 前检测到知识来源或最新策略已变化，已取消发送',
        ...
        if (reservedInLastHour >= immediateConfig.replyPolicy.maxRepliesPerHour) {
          await this.setStatus(record.id, 'needs_manual', {
            reason: '调用 DEMO_MESSAGE 前检测到最新每小时额度不足，已取消发送'
          })
          return 'needs_manual'
        }
      }
      // There is intentionally no await between the synchronous final guards above
      // and starting the DEMO_MESSAGE call, so config/index mutations cannot interleave here.
      const sendPromise = this.demoMessage.sendMessage({
        groupId: group.id,
        text: reply,
      ...
      await this.setStatus(record.id, 'sent', {
        reason: result.deduplicated ? 'DEMO_MESSAGE 幂等键命中，发送结果已确认' : '已发送',
