...
export class KnowledgeSynchronizer {
  ...
  private async readSource(source: KnowledgeSource): Promise<KnowledgeDocument[]> {
    switch (source.type) {
      ...
      case 'sampleLibrary-doc': {
        const route = /^https?:\/\//iu.test(source.routeOrUrl)
          ? await this.sampleLibrary.resolve(source.routeOrUrl)
          : source.routeOrUrl
        const document = await this.sampleLibrary.showDocument(route)
        return [toKnowledgeDocument(source, document.route, document.title, document.body, document.updatedAt)]
      }
      case 'sampleLibrary-book': {
        const entries = await this.sampleLibrary.listDocuments(source.namespace)
        const documents: KnowledgeDocument[] = []
        for (const entry of entries.slice(0, MAX_FILES_PER_SOURCE)) {
          const document = await this.sampleLibrary.showDocument(entry.route)
          documents.push(
            toKnowledgeDocument(
              source,
              document.route,
...
const htmlToText = (html: string): string =>
  normalizeText(
    html
      ...
      .replace(/<[^>]+>/gu, ' ')
      // A single pass prevents `&amp;lt;` from being decoded twice into `<`.
      .replace(/&(nbsp|amp|lt|gt|quot|#39);/giu, decodeHtmlEntity)
  )

const decodeHtmlEntity = (entity: string, name: string): string => {
