...
import {
  ...
  type DemoWorkflowRequirementCreateInput,
  type DemoWorkflowRequirementInput
...
const registerIpc = (): void => {
  ...
  trustedHandle(IPC_CHANNELS.saveConfig, async (value: AppConfig) => {
    ...
    if (approveLiveSending) {
      const approved = await confirmSensitiveAction({
        title: '确认开启真实自动回复',
        message: '开启后 LuckyTag 将向白名单示例消息群真实发送回复。',
  ...
  trustedHandle(
    ...
    async (kind: 'sampleMessaging' | 'sampleLibrary' | 'sampleauth') => {
  ...
  trustedHandle(IPC_CHANNELS.disconnectSampleAuth, async () => {
    const approved = await confirmSensitiveAction({
      title: '确认退出统一身份',
      message: '退出后 SampleAuth 身份连接将断开。',
      detail: '需要使用统一身份能力时，必须重新完成登录授权。',
      confirmLabel: '确认退出'
    })
    if (!approved) throw actionCancelledError()
    return requireDaemon().disconnectSampleAuth({ approved: true })
  })
  ...
  trustedHandle(IPC_CHANNELS.previewDemoWorkflowRequirement, async (value: DemoWorkflowRequirementInput) => {
    return requireDaemon().previewDemoWorkflowRequirement(assertDemoWorkflowRequirementInput(value))
  })
  trustedHandle(IPC_CHANNELS.createDemoWorkflowRequirement, async (value: DemoWorkflowRequirementCreateInput) => {
    return requireDaemon().createDemoWorkflowRequirement(assertDemoWorkflowRequirementCreateInput(value))
  })
  trustedHandle(IPC_CHANNELS.openDemoWorkflowRequirement, async (value: string) => {
    await shell.openExternal(assertDemoWorkflowRequirementUrl(value), { activate: true })
    return null
  })
