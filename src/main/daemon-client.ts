...
export const DAEMON_REQUEST_TIMEOUTS = Object.freeze({
  agentTest: 130_000,
  // SampleAuth intentionally allows a five-minute browser login window. Keep the
  ...
  demoWorkflowWorkflow: 10 * 60_000,
...
export class DaemonClient {
  ...
  authenticate(kind: 'sampleMessaging' | 'sampleLibrary' | 'sampleauth'): Promise<ConnectionStatus> {
    return this.request(
      'POST',
  ...
  disconnectSampleAuth(options: { approved?: boolean } = {}): Promise<DashboardSnapshot> {
    return this.request(
      'POST',
      '/v1/connections/sampleauth/disconnect',
      undefined,
      DEFAULT_REQUEST_TIMEOUT_MS,
      options.approved
        ? { [DAEMON_NATIVE_APPROVAL_HEADER]: DAEMON_NATIVE_APPROVAL.disconnectSampleAuth }
        : undefined
    )
  }
  ...
  previewDemoWorkflowRequirement(input: DemoWorkflowRequirementInput): Promise<DemoWorkflowRequirementPreview> {
    return this.request('POST', '/v1/demoWorkflow/preview', input, DAEMON_REQUEST_TIMEOUTS.demoWorkflowWorkflow)
  }

  createDemoWorkflowRequirement(input: DemoWorkflowRequirementCreateInput): Promise<DemoWorkflowRequirementResult> {
    return this.request('POST', '/v1/demoWorkflow/create', input, DAEMON_REQUEST_TIMEOUTS.demoWorkflowWorkflow)
  }
