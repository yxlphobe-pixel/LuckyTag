...
import { DimaRequirementPanel } from './components/DimaRequirementPanel'
...
const sectionMeta: Record<SectionId, { eyebrow: string; title: string; description: string }> = {
  ...
  dima: { eyebrow: 'Chat to Requirement', title: 'Dima需求', description: '从指定钉钉群聊中提炼需求，确认后写入目标 Dima 空间。' },
...
export function App(): React.JSX.Element {
  ...
  const [confirmOpenAuthDisconnect, setConfirmOpenAuthDisconnect] = useState(false)
  const [dimaPreview, setDimaPreview] = useState<DimaRequirementPreview | null>(null)
  const [dimaResult, setDimaResult] = useState<DimaRequirementResult | null>(null)
  ...
  useEffect(() => {
    if (!confirmLive && !confirmOpenAuthDisconnect) return undefined
    const handleEscape = (event: KeyboardEvent): void => {
      if (event.key !== 'Escape') return
      setConfirmLive(false)
      setConfirmOpenAuthDisconnect(false)
    }
    window.addEventListener('keydown', handleEscape)
    return () => window.removeEventListener('keydown', handleEscape)
  }, [confirmLive, confirmOpenAuthDisconnect])
  ...
  const addRemoteSource = (input: RemoteSourceInput): void => {
    if (!snapshot) return
    const source: KnowledgeSource = input.type === 'yuque-doc'
      ? { id: createId('yuque-doc'), type: input.type, label: input.label, routeOrUrl: input.routeOrUrl, enabled: true }
      : { id: createId('yuque-book'), type: input.type, label: input.label, namespace: input.namespace, enabled: true }
    void saveConfig({ ...snapshot.config, knowledgeSources: [...snapshot.config.knowledgeSources, source] }, '已添加语雀知识源')
  }
  ...
  const probeConnections = async (): Promise<void> => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        const available = result.data.filter((connection) => connection.kind !== 'ding-a1')
        const connected = available.filter((connection) => connection.state === 'connected').length
        pushToast('success', '连接检测完成', `${connected} / ${available.length} 项可用；钉钉 A1 为后续能力`)
  ...
  const authenticate = async (kind: 'dingtalk' | 'yuque' | 'openauth'): Promise<void> => {
    ...
    } else if (kind === 'openauth') {
      pushToast('info', '正在打开 OpenAuth 登录页', '请在浏览器中完成统一身份登录，LuckyTag 会等待登录结果')
    }
    try {
  ...
  const disconnectOpenAuth = async (): Promise<void> => {
    setConfirmOpenAuthDisconnect(false)
    beginBusy('disconnect-openauth')
    try {
      const result = await window.luckyTag.disconnectOpenAuth()
      if (result.ok) {
        setSnapshot(result.data)
        pushToast('success', 'OpenAuth 身份已断开', '本机 OpenAuth 会话已安全退出')
      } else pushToast('error', '无法断开 OpenAuth 身份', result.error.message)
    } catch (error) {
      pushToast('error', '无法断开 OpenAuth 身份', errorText(error))
    } finally {
      endBusy('disconnect-openauth')
    }
  }
  ...
  const previewDimaRequirement = async (input: DimaRequirementInput): Promise<void> => {
    beginBusy('dima-preview')
    setDimaPreview(null)
    setDimaResult(null)
    try {
      const result = await window.luckyTag.previewDimaRequirement(input)
      if (result.ok) {
        setDimaPreview(result.data)
        pushToast('success', '需求草稿已生成', `读取 ${result.data.messageCount} 条群消息，请检查后确认创建`)
      } else {
        pushToast('error', '无法生成 Dima 需求草稿', result.error.message)
      }
    } catch (error) {
      pushToast('error', '无法生成 Dima 需求草稿', errorText(error))
    } finally {
      endBusy('dima-preview')
    }
  }

  const createDimaRequirement = async (input: DimaRequirementCreateInput): Promise<void> => {
    beginBusy('dima-create')
    try {
      const result = await window.luckyTag.createDimaRequirement(input)
      if (result.ok) {
        setDimaResult(result.data)
        pushToast('success', 'Dima 需求创建成功', result.data.url)
      } else {
        pushToast('error', 'Dima 需求未创建', result.error.message)
      }
    } catch (error) {
      pushToast('error', 'Dima 需求未创建', errorText(error))
    } finally {
      endBusy('dima-create')
    }
  }

  const openDimaRequirement = async (url: string): Promise<void> => {
    try {
      const result = await window.luckyTag.openDimaRequirement(url)
      if (!result.ok) pushToast('error', '无法打开 Dima 需求', result.error.message)
    } catch (error) {
      pushToast('error', '无法打开 Dima 需求', errorText(error))
    }
  }
  ...
  const connectionBusy = isBusy('probe') || Object.keys(busyActions).some((action) => action.startsWith('auth-')) || isBusy('disconnect-openauth')
  ...
  const dimaBusyAction = isBusy('dima-preview') ? 'dima-preview' : isBusy('dima-create') ? 'dima-create' : null
  const enabledGroups = snapshot.config.groups.filter((group) => group.enabled).length
  const connectableConnections = snapshot.connections.filter((connection) => connection.kind !== 'ding-a1')
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="page-content">
          {section === 'overview' && (
            <div className="content-stack">
              ...
              <section className="metric-grid" aria-label="运行概览">
                ...
                <MetricCard icon="shield" label="可用连接" value={`${connectedCount} / ${connectableConnections.length}`} detail={connectedCount === connectableConnections.length ? '一期连接全部就绪' : '仍有连接待配置；A1 为后续能力'} tone="green" />
              ...
              <ConnectionGrid busy={connectionBusy} connections={snapshot.connections} onAuthenticate={(kind) => void authenticate(kind)} onDisconnectOpenAuth={() => setConfirmOpenAuthDisconnect(true)} onProbe={() => void probeConnections()} />
          ...
          {section === 'dima' && (
            <DimaRequirementPanel
              busyAction={dimaBusyAction}
              onAnalyze={(input) => void previewDimaRequirement(input)}
              onCreate={(input) => void createDimaRequirement(input)}
              onOpen={(url) => void openDimaRequirement(url)}
              onReset={() => {
                setDimaPreview(null)
                setDimaResult(null)
              }}
          ...
          {section === 'settings' && (
            <div className="content-stack">
              <ConnectionGrid busy={connectionBusy} compact connections={snapshot.connections} onAuthenticate={(kind) => void authenticate(kind)} onDisconnectOpenAuth={() => setConfirmOpenAuthDisconnect(true)} onProbe={() => void probeConnections()} />
              <div className="settings-grid">
                ...
                <section className="panel settings-card">
                  ...
                  <div><span className="section-kicker">关于</span><h2>LuckyTag</h2><p>macOS 本地个人分身助理。第一期专注于基于可信知识库的钉钉自动回复。</p></div>
                  <span className="version-chip">Reborn · 0.3.0</span>
                </section>
              </div>
      ...
      {confirmOpenAuthDisconnect && (
        <div className="modal-backdrop" role="presentation">
          <section aria-describedby="openauth-disconnect-description" aria-labelledby="openauth-disconnect-title" aria-modal="true" className="confirm-dialog" role="dialog">
            <span className="dialog-icon"><Icon name="warning" size={26} /></span>
            <h2 id="openauth-disconnect-title">断开 OpenAuth 身份？</h2>
            <p id="openauth-disconnect-description">这会退出本机全部 OpenAuth skill 会话，其他依赖 OpenAuth 的本地工具可能也需要重新登录。钉钉 DWS 与语雀 CLI 的独立登录态不会被清除。</p>
            <div className="dialog-actions"><button autoFocus className="button ghost" onClick={() => setConfirmOpenAuthDisconnect(false)} type="button">保持连接</button><button className="button danger" onClick={() => void disconnectOpenAuth()} type="button">确认断开</button></div>
          </section>
        </div>
      )}
