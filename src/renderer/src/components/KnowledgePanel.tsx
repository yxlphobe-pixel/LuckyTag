...
export type RemoteSourceInput =
  | { type: 'sampleLibrary-doc'; label: string; routeOrUrl: string }
  | { type: 'sampleLibrary-book'; label: string; namespace: string }
...
export function KnowledgePanel({ sources, stats, busy, onChooseLocal, onAddRemote, onToggle, onRemove, onSync }: KnowledgePanelProps): React.JSX.Element {
  const [remoteType, setRemoteType] = useState<'sampleLibrary-doc' | 'sampleLibrary-book'>('sampleLibrary-doc')
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>): void => {
    ...
    if (!value) return
    const sourceLabel = label.trim() || (remoteType === 'sampleLibrary-doc' ? '示例知识库文档' : '示例知识库知识库')
    if (remoteType === 'sampleLibrary-doc') onAddRemote({ type: remoteType, label: sourceLabel, routeOrUrl: value })
    ...
    <div className="content-stack">
      <section className="knowledge-hero panel">
        <div>
          ...
          <h2>让回复有据可查</h2>
          <p>添加本地资料或示例知识库内容。同步后，LuckyTag 只会基于已索引的知识生成回复。</p>
        </div>
      ...
      <div className="knowledge-layout">
        <section className="panel source-add-panel">
          ...
          <div className="source-divider"><span>或连接示例知识库</span></div>
          <div className="segmented" role="tablist" aria-label="示例知识库来源类型">
            <button aria-selected={remoteType === 'sampleLibrary-doc'} className={remoteType === 'sampleLibrary-doc' ? 'active' : ''} disabled={busy} onClick={() => setRemoteType('sampleLibrary-doc')} role="tab" type="button">单篇文档</button>
            <button aria-selected={remoteType === 'sampleLibrary-book'} className={remoteType === 'sampleLibrary-book' ? 'active' : ''} disabled={busy} onClick={() => setRemoteType('sampleLibrary-book')} role="tab" type="button">知识库</button>
          </div>
          <form className="remote-source-form" onSubmit={handleSubmit}>
            <label>
              <span>显示名称 <small>可选</small></span>
              <input disabled={busy} maxLength={120} onChange={(event) => setLabel(event.target.value)} placeholder={remoteType === 'sampleLibrary-doc' ? '如：产品 FAQ' : '如：团队知识库'} value={label} />
            </label>
            <label>
              <span>{remoteType === 'sampleLibrary-doc' ? '文档链接或路径' : '知识库 Namespace'}</span>
              <input disabled={busy} onChange={(event) => setRemoteValue(event.target.value)} placeholder={remoteType === 'sampleLibrary-doc' ? 'https://library.example.invalid/…' : 'team/book'} required value={remoteValue} />
            </label>
            <button className="button secondary full" disabled={busy || !remoteValue.trim()} type="submit"><Icon name="plus" size={15} /> 添加示例知识库来源</button>
          </form>
        </section>
        ...
        <section className="panel source-list-panel">
          ...
          {sources.length === 0 ? (
            <div className="empty-state compact-empty">
              ...
              <h3>还没有知识源</h3>
              <p>从左侧添加一个本地文件夹或示例知识库内容。</p>
            </div>
          ) : (
            <div className="source-list">
              {sources.map((source) => {
                const description = source.type === 'local-directory' ? source.path : source.type === 'sampleLibrary-doc' ? source.routeOrUrl : source.namespace
                const icon = source.type === 'local-directory' ? 'folder' : source.type === 'sampleLibrary-doc' ? 'link' : 'library'
                return (
