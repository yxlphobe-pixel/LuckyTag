...
const exampleModelUrl = 'https://api.example.invalid/v1'
...
const providerPresets: readonly ProviderPreset[] = [
  ...
  { id: 'sample-cloud', runtime: 'codex', label: '示例云服务', detail: 'OpenAI Responses 兼容', baseUrl: exampleModelUrl, authentication: 'api-key' },
...
export function ApplicationConfigurationPanel({
  ...
  const selectProvider = (preset: ProviderPreset): void => {
    ...
    setClearApiKey(false)
    setValidationError(null)
    if (!modelName && preset.id === 'sample-cloud') setModelName(modelSuggestions.codex[0] ?? '')
  }
