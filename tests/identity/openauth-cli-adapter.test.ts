...
import {
  ...
  ExecFileOpenAuthCommandRunner,
  OpenAuthCliAdapter,
  type OpenAuthCommandOptions,
  type OpenAuthCommandResult,
  type OpenAuthCommandRunner
} from '../../src/main/identity/openauth-cli-adapter'

const ok = (payload: unknown): OpenAuthCommandResult => ({
  exitCode: 0,
...
class RecordingOpenAuthRunner implements OpenAuthCommandRunner {
  readonly calls: Array<{
    executable: string
    args: readonly string[]
    options: OpenAuthCommandOptions
  }> = []
  readonly responses: Array<OpenAuthCommandResult | Promise<OpenAuthCommandResult>> = []
  ...
  async run(
    ...
    if (!response) throw new Error('测试未配置 OpenAuth CLI 响应')
    return response
  }
}
...
const createChildReadinessMarker = async (): Promise<{
  ...
  const directory = await mkdtemp(join(tmpdir(), 'luckytag-openauth-runner-'))
...
const runReadyTimeoutFixture = async (
  ...
  try {
    const runner = new ExecFileOpenAuthCommandRunner({ terminationGraceMs: 50 })
...
describe('OpenAuthCliAdapter', () => {
  it('parses a logged-in status into the renderer-safe identity allow-list', async () => {
    ...
    runner.responses.push(
      ok({
        ...
        iamExpiresAt: 2_000_000_000,
        ssoTicketExpiresAt: 2_000_003_600,
        accessToken: 'must-not-leak',
        ssoTicket: 'must-not-leak-either'
      })
    )

    const session = await new OpenAuthCliAdapter(runner).status()
  ...
  it('accepts the official exit-1 JSON contract for a logged-out status', async () => {
    ...
    await expect(new OpenAuthCliAdapter(runner).probe()).resolves.toEqual({
      authenticated: false
    })
  })
  ...
  it('runs browser login once for concurrent callers with --yes and a timeout beyond five minutes', async () => {
    const runner = new RecordingOpenAuthRunner()
    const response = deferred<OpenAuthCommandResult>()
    runner.responses.push(
      response.promise,
      ok({
        loggedIn: true,
        user: { name: 'Lucky', sno: '012345' },
        iamExpiresAt: 2_000_000_000,
        ssoTicketExpiresAt: null
      })
    )
    const adapter = new OpenAuthCliAdapter(runner)
    ...
    runner.responses.push(
      ok({ success: true, user: { sno: '012345' } }),
      ok({ loggedIn: true, user: { sno: '012345' }, iamExpiresAt: 2_000_000_000 })
    )
    await adapter.login()
    expect(runner.calls).toHaveLength(4)
  })

  it('refreshes an iam_expired session before considering interactive login', async () => {
    const runner = new RecordingOpenAuthRunner()
    runner.responses.push(
      {
        exitCode: 1,
        stdout: JSON.stringify({ loggedIn: false, reason: 'iam_expired' }),
        stderr: '',
        timedOut: false
      },
      ok({ success: true, user: { name: 'Lucky', sno: '012345', sub: 'iam:person:012345' } }),
      ok({
        loggedIn: true,
        user: { name: 'Lucky', sno: '012345' },
        iamExpiresAt: 2_000_000_000,
    ...
    await expect(new OpenAuthCliAdapter(runner).authenticate()).resolves.toMatchObject({
      authenticated: true,
      displayName: 'Lucky',
      employeeNumber: '012345'
    })
  ...
  it('falls back to browser login only after an iam_expired refresh fails', async () => {
    const runner = new RecordingOpenAuthRunner()
    runner.responses.push(
      {
        exitCode: 1,
  ...
  it('cancels and settles an in-flight login before logout so logout wins', async () => {
    const runner = new RecordingOpenAuthRunner()
    const loginResponse = deferred<OpenAuthCommandResult>()
    ...
    await expect(loginResult).resolves.toMatchObject({ code: 'OPENAUTH_CANCELLED' })
  ...
  it('uses the official idempotent logout JSON contract', async () => {
    ...
    await expect(new OpenAuthCliAdapter(runner).logout()).resolves.toBeUndefined()
  ...
  it.each([
    ...
    async ({ exitCode, reason }) => {
      ...
      await expect(new OpenAuthCliAdapter(runner).status()).rejects.toMatchObject({
        code: 'OPENAUTH_PROTOCOL'
      })
    }
  )
  ...
  it.each([
    {
      ...
      payload: { loggedIn: true, iamExpiresAt: 2_000_000_000 }
    },
    {
      label: 'missing expiry',
      payload: { loggedIn: true, user: { name: 'Lucky', sno: '012345' } }
    },
    {
      label: 'expired identity',
      payload: { loggedIn: true, user: { name: 'Lucky', sno: '012345' }, iamExpiresAt: 1 }
    }
  ...
  it('keeps arguments literal in the production runner (shell is disabled)', async () => {
    const runner = new ExecFileOpenAuthCommandRunner()
  ...
  it('removes inherited OPENAUTH_TICKET while preserving OPENAUTH_HOME', async () => {
    const runner = new ExecFileOpenAuthCommandRunner({
      environment: {
        PATH: process.env.PATH,
        OPENAUTH_TICKET: 'password-equivalent-ticket',
        OPENAUTH_HOME: '/private/tmp/luckytag-openauth-home'
      }
    })
    const result = await runner.run(
      process.execPath,
      [
        '-e',
        'process.stdout.write(JSON.stringify({ticket:process.env.OPENAUTH_TICKET??null,home:process.env.OPENAUTH_HOME??null}))'
      ],
      { timeoutMs: 5_000 }
    )
    ...
    expect(JSON.parse(result.stdout)).toEqual({
      ticket: null,
      home: '/private/tmp/luckytag-openauth-home'
    })
