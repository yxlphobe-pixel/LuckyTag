...
describe('Dima requirement validation', () => {
  it('normalizes the form and accepts an optional empty iteration', () => {
    expect(assertDimaRequirementInput({
      groupName: '  链路联调群  ',
      chatWindow: '24h',
      dimaSpace: ' 示例空间 ',
      iteration: '',
      templateUrl: 'https://project.alipay.com/space/W26000000001/sprint/detail?openWorkItemId=2026000000000000001'
    })).toMatchObject({
      groupName: '链路联调群',
      dimaSpace: '示例空间',
      iteration: ''
  ...
  it.each([
    'https://example.com/?openWorkItemId=2026000000000000001',
    'http://project.alipay.com/?openWorkItemId=2026000000000000001',
    'https://project.alipay.com/space/W26000000001'
  ...
  it('allows Markdown newlines in an edited draft but rejects non-Dima output links', () => {
    expect(assertDimaRequirementCreateInput({
      draftId: '123e4567-e89b-42d3-a456-426614174000',
      title: '修复链路异常',
      description: '## 需求内容\n- 修复问题'
    }).description).toContain('\n')
    expect(() => assertDimaRequirementUrl('https://example.com/workItem?id=1')).toThrow()
  })
})
...
describe('app config validation', () => {
  ...
  it('rejects C0/C1 controls in Yuque routes and namespaces', () => {
