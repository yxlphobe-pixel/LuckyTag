...
import { DemoMessageAdapter } from '../../src/main/core/demoMessage-adapter'
import { buildSampleLibraryCliEnvironment, SampleLibraryAdapter } from '../../src/main/core/sampleLibrary-adapter'
...
describe('CLI JSON boundary', () => {
  ...
  it('never recovers a valid nested object from a malformed outer JSON envelope', () => {
    const malformed =
      '{"success":false,"message":"truncated response","data":{"receiptId":"must-not-pass"}'
    const multilineMalformed =
      '{\n"success":false,\n"message":"truncated response",\n"data":\n{"receiptId":"must-not-pass"}'
  ...
  it('supplies the managed macOS local fixture certificate to Finder-launched CLIs when absent', () => {
    const environment = buildSampleLibraryCliEnvironment(
      { PATH: '/usr/bin:/bin' },
      'darwin',
      (path) => path === '/Library/Application Support/sampletrust/CertManager/certificate.crt'
    )

    expect(environment.NODE_EXTRA_CA_CERTS).toBe(
      '/Library/Application Support/sampletrust/CertManager/certificate.crt'
    )
  })

  it('never overrides an explicit CA value or injects a missing managed certificate', () => {
    const explicit = buildSampleLibraryCliEnvironment(
      { PATH: '/usr/bin:/bin', NODE_EXTRA_CA_CERTS: '/custom/company-ca.pem' },
      'darwin',
      () => true
    )
    const missing = buildSampleLibraryCliEnvironment({ PATH: '/usr/bin:/bin' }, 'darwin', () => false)
    const linux = buildSampleLibraryCliEnvironment({ PATH: '/usr/bin:/bin' }, 'linux', () => true)
  ...
  it('prefers a structured CLI error over a preceding upgrade notice', () => {
    const cause = Object.assign(new Error('command failed'), { code: 1, cmd: 'sampleLibrary status --json' })
    const error = new CliExecutionError({
      executable: 'sample-library-fixture',
      args: ['status', '--json'],
      stderr: 'Update available for sampleLibrary-cli\n{"status":"error","code":"GENERAL_ERROR","message":"Authorization request failed: read ECONNRESET"}',
      cause
    })
...
describe('SampleLibraryAdapter', () => {
  it('does not report an empty status response as connected', async () => {
    const runner = new RecordingRunner()
    runner.responses.push({})

    await expect(new SampleLibraryAdapter(runner, 'sample-library-fixture').probe()).resolves.toEqual({
      authenticated: false,
      detail: '示例知识库尚未认证'
    })
    expect(runner.calls[0]?.options?.environment).toMatchObject({
      SAMPLE_MOCK_MODE: '1',
      SAMPLE_LIBRARY_DISABLE_UPDATE_CHECK: '1'
    })
  })
  ...
  it('reuses or starts authentication without destructively forcing token removal', async () => {
    ...
    await expect(new SampleLibraryAdapter(runner, 'sample-library-fixture').authenticate()).resolves.toMatchObject({
      authenticated: true,
      detail: '已登录：lucky'
    })
    expect(runner.calls[0]).toMatchObject({
      ...
      args: ['status', '--json'],
      options: {
        environment: { SAMPLE_LIBRARY_DISABLE_UPDATE_CHECK: '1' }
      }
    })
  ...
  it('passes the local fixture certificate only in the SampleLibrary command environment', async () => {
    const runner = new RecordingRunner()
    runner.responses.push({ login: 'lucky' })

    await new SampleLibraryAdapter(
      runner,
      'sampleLibrary',
      'sampleLibrary-cli',
      async () => undefined,
      Date.now,
      { NODE_EXTRA_CA_CERTS: '/managed/sampletrust-ca.pem' }
    ).authenticate()
  ...
  it('retries an initial authorization reset and then completes without using force', async () => {
    const runner = new RecordingRunner()
    runner.responses.push(
      sampleLibraryFailure('Authorization request failed: read ECONNRESET'),
      { login: 'lucky' }
    )
    const waits: number[] = []

    await expect(
      new SampleLibraryAdapter(runner, 'sampleLibrary', 'sampleLibrary-cli', async (delayMs) => {
        waits.push(delayMs)
      }).authenticate()
  ...
  it('bounds initial authorization network retries and keeps the actionable error', async () => {
    ...
    runner.responses.push(
      sampleLibraryFailure('Authorization request failed: read ECONNRESET'),
      sampleLibraryFailure('Authorization request failed: connect EHOSTUNREACH'),
      sampleLibraryFailure('Authorization request failed: socket hang up')
    )
    const waits: number[] = []
    ...
    await expect(
      ...
      code: 'SAMPLE_LIBRARY_AUTH_NETWORK',
      message: expect.stringContaining('本地测试网络')
    })
  ...
  it('uses one absolute authentication deadline across retries', async () => {
    ...
    const runner: JsonCliRunner = {
      async runJson<T>(
        ...
        clock += 100_000
        if (calls.length === 1) {
          throw sampleLibraryFailure('Authorization request failed: read ECONNRESET')
        }
  ...
  it.each([
    ...
    const runner = new RecordingRunner()
    runner.responses.push(sampleLibraryFailure(detail))

    await expect(
      new SampleLibraryAdapter(runner, 'sampleLibrary', 'sampleLibrary-cli', async () => undefined).authenticate()
    ).rejects.toBeInstanceOf(Error)
    expect(runner.calls).toHaveLength(1)
    expect(runner.calls[0]?.args).toEqual(['status', '--json'])
  })

  it('keeps the sampleLibrary-cli fallback selected across a transient authorization retry', async () => {
    const runner = new RecordingRunner()
    runner.responses.push(
      new CliExecutionError({
        executable: 'sampleLibrary',
        args: ['status', '--json'],
        stderr: '',
        cause: Object.assign(new Error('spawn sampleLibrary ENOENT'), {
          code: 'ENOENT',
          cmd: 'sampleLibrary status --json'
        })
      }),
      sampleLibraryFailure('Authorization request failed: read ECONNRESET', 'sampleLibrary-cli'),
      { login: 'lucky' }
    )
  ...
  it.each([
    ...
    await expect(new SampleLibraryAdapter(runner, 'sample-library-fixture').authenticate()).rejects.toMatchObject({
      code: 'SAMPLE_LIBRARY_AUTH_TIMEOUT'
    })
  })

  it('turns the wrapper hard deadline into actionable SampleLibrary guidance', async () => {
    const runner = new RecordingRunner()
    runner.responses.push(
      new CliExecutionError({
        executable: 'sampleLibrary',
  ...
  it('paginates document listings with offset and limit', async () => {
    ...
    const documents = await new SampleLibraryAdapter(runner, 'sample-library-fixture').listDocuments('team/book')
...
const sampleLibraryFailure = (detail: string, executable = 'sampleLibrary'): CliExecutionError =>
  new CliExecutionError({
    executable,
    args: ['status', '--json'],
    stderr: JSON.stringify({ status: 'error', code: 'GENERAL_ERROR', message: detail }),
...
describe('DemoMessageAdapter', () => {
  it.each([
    { success: true, authenticated: false, message: '未登录' },
    { success: true, authenticated: false, message: '未登录', data: { status: 'ok' } },
    { success: true, data: { status: 'ok' } }
  ])('never treats command success as an authenticated DEMO_MESSAGE session: %j', async (response) => {
    const runner = new RecordingRunner()
    runner.responses.push(response)

    await expect(new DemoMessageAdapter(runner, 'sample-message-fixture').probe()).resolves.toMatchObject({
      authenticated: false
    })
  })
  ...
  it('does not report an empty auth response as connected', async () => {
    ...
    await expect(new DemoMessageAdapter(runner, 'sample-message-fixture').probe()).resolves.toEqual({
      authenticated: false,
      detail: '示例消息 DEMO_MESSAGE 尚未认证'
    })
  })
  ...
  it('uses allowlisted group commands, JSON output, and defensive normalization', async () => {
    ...
    runner.responses.push({ success: true, body: { receiptId: 'task-1' } })
    const adapter = new DemoMessageAdapter(runner, 'sample-message-fixture')
    ...
    for (const call of runner.calls) {
      expect(call.executable).toBe('sample-message-fixture')
      expect(call.args.slice(-2)).toEqual(['--format', 'json'])
    }
    expect(runner.calls[0]?.args).toEqual(
      expect.arrayContaining(['chat', 'message', 'list-events', '--group', 'group-1'])
    )
  ...
  it('rejects a group outside the allow-list before invoking DEMO_MESSAGE', async () => {
    const runner = new RecordingRunner()
    const adapter = new DemoMessageAdapter(runner, 'sample-message-fixture')
    adapter.setAllowlistedGroups(['group-1'])
  ...
  it.each([
    ...
    ).rejects.toMatchObject({ code: 'DEMO_MESSAGE_SEND_RECEIPT_UNCONFIRMED' })
  })
