...
class FakeDws implements DwsClient {
  allowlist = new Set<string>()
  mentions: DwsMessage[] = []
  history: DwsMessage[] = []
  historyResponses: DwsMessage[][] = []
  historyCalls = 0
  ...
  async sendMessage(input: { groupId: string; text: string; uuid: string }): Promise<DwsSendResult> {
...
const knowledgeChunks: KnowledgeChunk[] = [
  {
    ...
    sourceLabel: '配置手册',
    title: '钉钉自动回复配置',
    text: '钉钉自动回复必须先配置群白名单，并建议先开启 dry-run 观察预览。',
    position: 0
  }
]

const baseMessage = (overrides: Partial<DwsMessage> = {}): DwsMessage => ({
  id: 'msg-1',
  groupId: 'group-1',
  text: '钉钉自动回复怎么配置？',
...
describe('WorkerEngine safety state machine', () => {
  ...
  it('does no polling or sending while the global policy switch is disabled', async () => {
    ...
    harness.dws.mentions = [baseMessage()]
    harness.dws.history = [baseMessage()]

    const summary = await harness.worker.runOnce()

    expect(summary.note).toContain('总开关已关闭')
    expect(harness.dws.allowlist.size).toBe(0)
    expect(harness.dws.historyCalls).toBe(0)
    expect((await harness.store.read()).replies).toHaveLength(0)
    expect(harness.dws.sendCalls).toHaveLength(0)
  })
  ...
  it('marks all existing mentions ignored on the first scan', async () => {
    ...
    harness.dws.mentions = [message]
    harness.dws.history = [message]
  ...
  it('does not redeliver the same mention and uses a stable DWS UUID', async () => {
    const harness = await createHarness()
    const message = baseMessage()
    harness.dws.mentions = [message]
    harness.dws.history = [message]
    ...
    expect(harness.dws.sendCalls).toHaveLength(1)
    expect(harness.dws.sendCalls[0]?.uuid).toMatch(/^[0-9a-f-]{36}$/u)
  ...
  it('retries an ambiguous send only after lease expiry with the exact same UUID', async () => {
    ...
    harness.dws.mentions = [message]
    harness.dws.history = [message]
    harness.dws.failuresRemaining = 1
    ...
    expect((await harness.worker.runOnce()).sent).toBe(1)
    expect(harness.dws.sendCalls).toHaveLength(2)
    expect(harness.dws.sendCalls[0]?.uuid).toBe(harness.dws.sendCalls[1]?.uuid)
  })
  ...
  it('fails closed when recovery history identifies a persisted pending message as self-authored', async () => {
    ...
    harness.dws.history = [ownMessage]
  ...
  it('hands off conservatively when another human has spoken after the question', async () => {
    ...
    harness.dws.mentions = [question]
    harness.dws.history = [question, interruption]
  ...
  it('rechecks immediately before sending and cancels on a TOCTOU human interruption', async () => {
    ...
    harness.dws.mentions = [question]
    harness.dws.historyResponses = [[question], [question, interruption]]
  ...
  it('honors a switch back to dry-run during the final pre-send history check', async () => {
    ...
    const question = baseMessage()
    harness.dws.mentions = [question]
    harness.dws.history = [question]
    harness.dws.onHistoryCall = (callNumber) => {
  ...
  it('counts ambiguous sends against the hourly budget while allowing same-record retries', async () => {
    ...
    harness.dws.mentions = [first, second]
    harness.dws.historyResponses = [[first], [first], [second], [second]]
    harness.dws.failuresRemaining = 1
  ...
  it('keeps unfinished work and recent sends while evicting the oldest terminal audit records', async () => {
    ...
    harness.dws.mentions = [baseMessage({ id: 'first-scan-trigger' })]
    ...
    harness.dws.mentions = [nextQuestion]
    harness.dws.history = [nextQuestion]
  ...
  it('allows the outbox to exceed 5000 rather than dropping pending or sending records', async () => {
    ...
    harness.dws.mentions = [baseMessage({ id: 'overflow-trigger' })]
  ...
  it('hands non-text or empty-content mentions to a human', async () => {
    ...
    harness.dws.mentions = [attachment]
  ...
  it('stops after three ambiguous sends and hands the record to a human', async () => {
    ...
    harness.dws.mentions = [question]
    harness.dws.history = [question]
    harness.dws.failuresRemaining = 3
    ...
    expect(harness.dws.sendCalls).toHaveLength(3)
    expect(new Set(harness.dws.sendCalls.map((call) => call.uuid)).size).toBe(1)
  })

  it('ignores looped-back messages sent by the current account', async () => {
    const harness = await createHarness()
    const ownMessage = baseMessage({ isSelf: true })
    harness.dws.mentions = [ownMessage]
    harness.dws.history = [ownMessage]
  ...
  it('continues a sent reply for the same sender within the follow-up window', async () => {
    ...
    harness.dws.history = [question, followUp]
    const summary = await harness.worker.runOnce()
    const state = await harness.store.read()
