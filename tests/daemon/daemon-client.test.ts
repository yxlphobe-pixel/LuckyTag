...
import { OPENAUTH_LOGIN_TIMEOUT_MS } from '../../src/main/identity/openauth-cli-adapter'
...
describe('DaemonClient Unix Domain Socket transport', () => {
  ...
  it('为交互认证与长任务保留足够的绝对截止时间', () => {
    expect(DAEMON_REQUEST_TIMEOUTS.authentication).toBeGreaterThanOrEqual(
      OPENAUTH_LOGIN_TIMEOUT_MS + 30_000
    )
  ...
  it('只在 Main 明确批准时为敏感 Daemon 请求附带原生批准断言', async () => {
    ...
    await client.disconnectOpenAuth({ approved: true })
    ...
    expect(approvals).toEqual([
      ...
      DAEMON_NATIVE_APPROVAL.disconnectOpenAuth,
      DAEMON_NATIVE_APPROVAL.enableLiveSending,
      DAEMON_NATIVE_APPROVAL.enableLiveSending
    ])
  })
...
const createFixture = async (
  ...
  const server = createServer((request, response) => {
    ...
    if (
      ...
      request.url === '/v1/connections/openauth/disconnect' ||
