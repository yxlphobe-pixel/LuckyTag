...
const roots: string[] = []
const runtimeFixtureSource = fileURLToPath(new URL('../fixtures/agent-runtime-fixture.mjs', import.meta.url))
...
describe.sequential('IsolatedAgentRunner', () => {
  ...
  it.each(['codex', 'claude-code'] as const)('%s 超时后终止进程组并回收会话目录', async (runtime) => {
    ...
    try {
      ...
      await expect(operation).rejects.toMatchObject({ code: 'AGENT_TIMEOUT' })
      expect(await readdir(join(fixture.dataRoot, 'workers'))).toEqual([])
...
describe.sequential('MacKeychainCredentialStore', () => {
  it('通过 stdin 写入 Key，进程参数不包含明文，并按 Runtime + URL 隔离', async () => {
    ...
    const serviceIndex = observedArgs.indexOf('-s')
    expect(observedArgs[serviceIndex + 1]).toBe('com.luckytag.agent.codex.0a8588be33f4d51314f80cd2')
    expect(observedArgs[serviceIndex + 1]).not.toContain('models.example.test')
...
const createRuntimeFixture = async (
  ...
  const executableName = `${runtime === 'codex' ? 'codex' : 'claude'}-${hangs ? 'hang-' : ''}fixture.mjs`
  const executable = join(root, executableName)
  await copyFile(runtimeFixtureSource, executable)
  await chmod(executable, 0o700)
